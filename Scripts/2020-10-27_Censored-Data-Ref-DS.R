...
working_data %<>%
  group_by(date, year, month, day, Parameter, Parameter.Sep, Fraction, Compound.Type, Guideline.Unit, 
           Detection.Limit, Below.RDL, Sample.Type) %>%
...
table.1 <- working_data %>% group_by(Parameter, Parameter.Sep, Fraction, Sample.Type) %>%
...
table.2 <- cens_data %>% group_by(Parameter, Parameter.Sep, Fraction, Sample.Type) %>%
  summarise(N.ND = length(Result),
            ND.MIN = min(Result),
            ND.MAX = max(Result))

table.1 %<>% left_join(table.2, by = c("Parameter", "Parameter.Sep", "Fraction", 'Sample.Type'))

table.1 %<>% arrange(Parameter.Sep, Fraction, Site.Order, .by_group = TRUE)

write.csv(table.1, file="tables\\2020-10-26_Table-1_Summary-Stats-ReferenceDownstream_No-Censor.csv")

# Determine non-detects by station to select method.
# Count samples that are D vs ND
method_selection <- working_data %>%
  group_by(Parameter, Parameter.Sep, Fraction, Sample.Type) %>%
...
# Join to data set
working_data %<>% left_join(method_selection, by = c("Parameter", "Parameter.Sep", "Fraction", 'Sample.Type'))

method_selection%<>% arrange(Parameter.Sep, Fraction, Sample.Type, .by_group = TRUE)

write.csv(method_selection, file="tables\\2020-10-26_Method-Selection-Ref-DS.csv")


# Create Table 6.1 (Breakdown by method)
method_summary <- method_selection %>%
  group_by(METHOD, Parameter, Parameter.Sep, Fraction) %>%
  summarise(STN.COUNT = length(unique(Sample.Type)),
            SMP.COUNT = sum(N))

write.csv(method_summary, file="tables\\2020-10-26_Method-Summary-Ref-DS.csv")
...
summary_stats %<>% left_join(select(table.1, Parameter, Parameter.Sep, Fraction, Sample.Type, 
                             N.ND, unit)) %>%
                    mutate(PROP.N = signif(r_n / N.ND), 3)


summary_stats %<>% arrange(Parameter, Parameter.Sep, Fraction, Sample.Type, .by_group = TRUE)

write.csv(summary_stats, file="tables\\2020-10-26_Summary-Stats-Censored-Ref-DS.csv")
